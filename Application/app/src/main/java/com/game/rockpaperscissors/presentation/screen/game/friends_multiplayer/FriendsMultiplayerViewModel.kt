...
@HiltViewModel
class FriendsMultiplayerViewModel @Inject constructor(
    context: Context
) : ViewModel() {
    private val TAG = "FriendsMultiplayerGameViewModel"
    ...
    fun startGame(rounds: Int, enemyName: String,callback: (Boolean) -> Unit) {
        callback(false)

        searchLobby(enemyName){ lobby, error ->
            if(lobby != null){
                lobbyId.value = lobby
                foundLobby(lobby, enemyName){
                    callback(it)
                }
            }else if (error != null){
                createLobby(rounds){
    ...
    fun exitPlayerSearch(){
        ...
        val lobbyRef = database.child("reserved_lobby").child(lobbyId.value)
...
//    reserved_lobby
//    classic_online_games
    private fun searchLobby(userName: String, callback: (String?, String?) -> Unit){
        val lobbyRef = database.child("reserved_lobby")

        val query: Query = lobbyRef.orderByChild("host").equalTo(userName)
        ...
        query.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                ...
                for(lobbySnapshot in dataSnapshot.children){
                    ...
                    if(currentTime - createdTime > 2 * 60 * 60 * 1000){
                        if (lobbyKey != null) {
    ...
    private fun waitForPlayer(lobbyId: String, callback: (String?, String?) -> Unit){
        val valueRef = database.child("reserved_lobby").child(lobbyId).child("foundPlayer")
    ...
    fun updatePlayerState(isReady: Boolean, selection: SelectionType?){
        ...
        val gameRef = database.child("classic_online_games").child(gameId.value).child("round${_currentRound.value}")
